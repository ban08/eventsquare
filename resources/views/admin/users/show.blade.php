                    ...
                    <div class="relative group">
                        <div class="w-32 h-32 rounded-2xl border-4 border-white bg-white shadow-lg overflow-hidden relative z-10">
                            @if($user->profile && $user->profile->photo_url)
                                <img src="{{ Str::startsWith($user->profile->photo_url, 'http') ? $user->profile->photo_url : (Str::startsWith($user->profile->photo_url, 'storage/') ? asset($user->profile->photo_url) : asset('storage/' . $user->profile->photo_url)) }}" alt="{{ $user->name }}" class="w-full h-full object-cover">
                            @else
